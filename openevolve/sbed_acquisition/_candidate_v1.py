...
def _acquire(self) -> float:
    ...
    explore_prob = 0.02 + 0.08 * decay
    if rand_val < explore_prob:
        ...
        if n_obs >= 5:
            ...
            if valid_dip_centers:
                unique_centers = sorted(set(valid_dip_centers))
                counts = getattr(self, "_dual_window_counts", None)
                if len(unique_centers) >= 2 and counts is not None:
                    target_idx = 0 if counts.get("left", 0) <= counts.get("right", 0) else -1
                    center = float(unique_centers[target_idx])
                else:
