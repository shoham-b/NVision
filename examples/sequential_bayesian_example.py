...
def simulate_nv_odmr_signal(frequency: float, true_params: Dict[str, float], noise_level: float = 0.05) -> float:
    """
...
def run_measurement_sequence(locator, domain: tuple, true_params: Dict[str, float], 
                           max_measurements: int = 50) -> List[Obs]:
    """
...
def compare_strategies():
    ...
    print(f"Sequential Bayesian Design:")
...
def visualize_adaptive_strategy(history: List[Obs], result: Dict[str, Any], title: str):
    ...
    true_signal = []
    for f in freq_range:
        lorentzian = true_params['amplitude'] * (true_params['linewidth']/2)**2 / \
    ...
    ax1.errorbar([f/1e9 for f in frequencies], intensities, yerr=uncertainties, 
                fmt='ro', label='Measurements', alpha=0.8)
    ax1.axvline(result['x1']/1e9, color='red', linestyle='--', alpha=0.8, 
               label=f'Estimated peak: {result["x1"]/1e9:.6f} GHz')
    ax1.axvline(true_params['frequency']/1e9, color='black', linestyle=':', alpha=0.8,
    ...
    scatter = ax2.scatter([f/1e9 for f in frequencies], measurement_order, 
                         c=measurement_order, cmap='viridis', s=50)
...
def demonstrate_information_gain():
    ...
    plt.axvline(optimal_freq/1e9, color='red', linestyle='--', 
...
def main():
    ...
    fig1 = visualize_adaptive_strategy(sequential_history, sequential_result, 
                                     "Sequential Bayesian Experiment Design")
    
    # Plot Grid Scan results
    fig2 = visualize_adaptive_strategy(grid_history, grid_result, 
                                     "Conventional Grid Scan")
