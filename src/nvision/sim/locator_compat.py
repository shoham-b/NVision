...
class LegacyLocatorShim(Locator):
    """Adapts batched locators to the legacy single-repeat interface."""

    def __init__(self, locator: Locator, repeat_id: int = 0):
    ...
    def _prepare_history(
        ...
        if history is None:
            ...
            history_df = pl.DataFrame(list(history))
        ...
        if "step" not in history_df.columns:
            history_df = history_df.with_row_count("step")
    ...
    def propose_next(self, history: pl.DataFrame, scan) -> float:
        prepared = self._prepare_history(history)
        proposals = self._locator.propose_next(prepared, self._repeats_df, scan)
        row = self._extract_row(proposals)
        if row.is_empty() or "x" not in row.columns:
            return float("nan")
        return float(row.get_column("x")[0])

    def should_stop(self, history: pl.DataFrame, scan) -> bool:
        prepared = self._prepare_history(history)
        decisions = self._locator.should_stop(prepared, self._repeats_df, scan)
        row = self._extract_row(decisions)
        stop = bool(row.get_column("stop")[0]) if not row.is_empty() else False
...
def wrap_legacy(locator: Locator) -> Locator:
    """Return a locator compatible with the legacy single-repeat runner."""

    if isinstance(locator, LegacyLocatorShim):
        return locator

    sig = inspect.signature(locator.propose_next)
    if "repeats" in sig.parameters:
        wrapped = LegacyLocatorShim(locator)
        wrapped.__wrapped_locator__ = locator
        return wrapped
    return locator
