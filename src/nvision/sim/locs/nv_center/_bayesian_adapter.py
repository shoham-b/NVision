"""Temporary adapter for NVCenterSequentialBayesianLocator to batched interface.

This module provides a compatibility layer that wraps the existing per-repeat
Bayesian locator to work with the new batched API. A full batched implementation
will be developed in a future iteration.
"""
...
@dataclass
class NVCenterSequentialBayesianLocatorBatched(Locator):
    """Batched adapter for the Sequential Bayesian locator.

    This adapter maintains one instance of the original per-repeat locator for each
    repeat_id and delegates calls to them individually. It is not truly vectorized,
