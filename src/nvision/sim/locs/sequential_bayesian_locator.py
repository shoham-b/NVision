...
class SequentialBayesianLocator:
    ...
    def odmr_model(self, frequency: float, params: Dict[str, float]) -> float:
        ...
        lorentzian = A * (gamma/2)**2 / ((frequency - f0)**2 + (gamma/2)**2)
    ...
    def update_posterior(self, measurement: Dict[str, float]):
        ...
        self.current_estimates['uncertainty'] = np.sqrt(
            np.sum((self.freq_grid - self.current_estimates['frequency'])**2 * self.freq_posterior)
        )
    ...
    def mutual_information_criterion(self, test_frequency: float) -> float:
        ...
        """
        # Current variance in frequency estimate
        current_var = np.sum((self.freq_grid - self.current_estimates['frequency'])**2 * self.freq_posterior)
        ...
        for i, freq in enumerate(self.freq_grid):
            ...
            # Simplified sensitivity calculation (derivative approximation)
            # In full implementation, would compute actual gradient
            df_dfreq = abs(test_frequency - freq) / self.current_estimates['linewidth']
            sensitivity += self.freq_posterior[i] * df_dfreq**2
    ...
    def propose_next(self, history: Sequence[Obs], domain: Tuple[float, float]) -> float:
        ...
        """
        # Convert history to internal format and update posterior
        for obs in history:
            measurement = {
                'frequency': obs.x,
            ...
            # Only process new measurements
            if not any(m['frequency'] == measurement['frequency'] for m in self.measurement_history):
                self.update_posterior(measurement)
        
        # Early measurements: explore domain bounds and center for initialization
        if len(self.measurement_history) < 3:
            candidates = [domain[0], np.mean(domain), domain[1]]
            tested_freqs = {m['frequency'] for m in self.measurement_history}
            for candidate in candidates:
                if candidate not in tested_freqs:
                    return candidate
                    
        # Use acquisition function to find optimal next measurement
        domain_low, domain_high = domain
        
        def negative_utility(freq):
            ...
            try:
                ...
                warnings.warn(f"Error in utility calculation: {e}")
                return 0.0  # Neutral utility if calculation fails
        
        # Optimize acquisition function over the domain
        try:
            ...
            warnings.warn(f"Optimization failed: {e}, using fallback strategy")
            # Fallback: measure near current best estimate
    ...
    def finalize(self, history: Sequence[Obs]) -> Dict[str, float]:
        ...
        for i in range(1, len(posterior_smooth)-1):
            if (posterior_smooth[i] > posterior_smooth[i-1] and 
                posterior_smooth[i] > posterior_smooth[i+1] and
                posterior_smooth[i] > threshold):
                peaks.append(i)
        ...
        if n_peaks == 0:
            ...
            return {
                ...
                "x2": self.freq_grid[peak_strengths[1][0]] if len(peak_strengths) > 1 else self.freq_grid[peak_strengths[0][0]],
                "uncert": self.current_estimates['uncertainty']
            }