...
def test_sweep_fit_asymmetric_triplet_shallow_line_hidden():
    ...
    coverage.py's line tracer makes it slow enough to blow past this test's
    already-bumped timeout. CI additionally runs under ``pytest -n auto``
    (pytest-xdist), so this test competes with every other worker process for
    CPU on a shared runner; ``no_cover`` alone wasn't enough headroom there
    (still timed out at >180s), hence the further bump to 300s.
