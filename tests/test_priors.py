...
def test_smc_belief_initializes_with_sin2_and_gaussian_priors():
    ...
    # "frequency" is fixed (a known, calibrated instrument constant -- see
    # NVCenterLorentzianModel(with_fixed_frequency=True), the default used by
    # nv_center_smc_belief), so it is not a particle/inferred dimension and has
    # no sin^2(k f) particle distribution to check.
    assert "frequency" not in belief._param_names
    assert "frequency" in belief.physical_param_bounds
