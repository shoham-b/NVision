...
def test_nv_center_lorentzian_no_zeeman_has_three_parameters():
    """Explicit with_zeeman_splitting=False gives single-dip 2-free-param model; frequency is fixed."""
    rng = random.Random(11)
...
def test_nv_center_saturation_voigt_default_has_zeeman_parameters():
    ...
    frequency is fixed (a known, calibrated zero-field reference) and c_max is not
    a parameter either -- it's a fixed constant (NV_SATURATION_C_MAX). Neither is
...
def _dip_cluster_extent(variant: str, sig: TrueSignal) -> tuple[float, float]:
    """Worst-case (outer_lo, outer_hi) of the full dip cluster for one drawn signal."""
    params = sig.parameter_values()
    # frequency is fixed (not a free parameter) -- read it via get_param_value's
    # fallback to typed_parameters instead of the free-parameter dict.
