...
def test_bayesian_sbed_nv_updates_with_normalized_probe_and_physical_signal():
    ...
    # "frequency" is fixed (not a free/inferred parameter) by default and therefore
    # absent from true_signal.parameter_names -- add its bounds back explicitly and
    # tell the builder to infer it, since that's exactly what this test exercises.
    pb = {name: true_signal.get_param_bounds(name) for name in true_signal.parameter_names}
    pb["frequency"] = (x_min, x_max)
    cfg = {
...
def _make_unit_cube_nv_model():
    ...
    # with_fixed_frequency=False: these tests exercise the fast/exact dispatch
    # mechanism over all 5 declared physical parameters (see param_arrays below),
    # not the "frequency" fixed-instrument-constant feature.
