...
def test_bayesian_sbed_nv_updates_with_normalized_probe_and_physical_signal():
    # The SBED locator draws from the global `np.random` state (exploration/dip-bias
    # branches, EIG tie-breaks), so it must be seeded here too — otherwise this test's
    # outcome depends on how much global numpy random state prior tests consumed.
    np.random.seed(11)
