...
def test_focus_window_automatic_narrowing_during_resampling():
    ...
    # Narrowing is gated behind a warm-up period (NVISION_MIN_STEPS_BEFORE_NARROWING,
    # default 8) so the filter doesn't focus prematurely on a multi-modal posterior.
    smc._step_count = 8
...
def test_shoulder_based_narrowing_single_dip():
    """Window should shrink around a single dip once the posterior has localized it.

    Narrowing is driven by the particle distribution (the "Unified Active-Range
    Union Focusing" in ``UnitCubeSMCMarginalDistribution._resample``), not by
    re-scanning raw observations, so the particles here are set up as SMC would
    leave them after importance-weighting/resampling against dip-shaped
    observations: concentrated tightly around the true dip centre.
    """
    np.random.seed(0)
    ...
    u_scale = dip_sigma / (freq_hi - freq_lo)
    smc._particles[:, 0] = np.clip(np.random.normal(loc=u_center, scale=u_scale, size=1000), 0.0, 1.0)
...
def test_shoulder_based_narrowing_connected_dips():
    """A bimodal posterior (two adjacent dips) must keep both modes inside the window."""
    np.random.seed(0)
    freq_lo, freq_hi = 2.7e9, 2.8e9
    smc = _make_smc(freq_lo, freq_hi)
    ...
    particles = np.concatenate(
        [
            np.random.normal(loc=u_a, scale=u_scale, size=half),
            np.random.normal(loc=u_b, scale=u_scale, size=1000 - half),
        ]
    )
