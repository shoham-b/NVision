...
def test_focus_window_automatic_narrowing_during_resampling():
    ...
    # global measurements have been made; simulate that we're past the delay.
    smc._step_count = 8
...
def test_shoulder_based_narrowing_single_dip():
    """A posterior converged tightly around one dip should narrow the window around it.

    Narrowing is driven by the particle posterior (Unified Active-Range Union
    Focusing), not by raw observations directly, so the particles here are set
    to the tight single-mode distribution a converged filter would produce
    after observing a single dip.
    """
    # A small fraction of particles are randomly rejuvenated across the full
    # domain on every resample (to guard against premature filter collapse),
    # so the resulting window width is inherently noisy run-to-run — seed for
    # a reproducible draw.
    np.random.seed(0)
    ...
    smc._particles[:, 0] = np.clip(np.random.normal(loc=u_center, scale=1e-4, size=1000), 0.0, 1.0)
...
def test_shoulder_based_narrowing_connected_dips():
    """A posterior split between two nearby, unresolved dips should still narrow, covering both.

    Narrowing is driven by the particle posterior (Unified Active-Range Union
    Focusing), so the particles here are bimodal — split between the two dip
    centres — mimicking the residual ambiguity a filter would retain when two
    dips are too close to disambiguate.
    """
    # A small fraction of particles are randomly rejuvenated across the full
    # domain on every resample (to guard against premature filter collapse),
    # so the resulting window width is inherently noisy run-to-run — seed for
    ...
    half = smc.num_particles // 2
    particles_a = np.random.normal(loc=u_a, scale=1e-4, size=half)
    particles_b = np.random.normal(loc=u_b, scale=1e-4, size=smc.num_particles - half)
    smc._particles[:, 0] = np.clip(np.concatenate([particles_a, particles_b]), 0.0, 1.0)
