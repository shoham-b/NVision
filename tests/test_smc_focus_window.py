...
def test_focus_window_automatic_narrowing_during_resampling():
    ...
    # steps have elapsed, so simulate having already run past that point.
    smc._step_count = 8
...
def test_shoulder_based_narrowing_single_dip():
    """Window should shrink to span the particle posterior around a single dip, not stay at the full span.

    ``_resample()`` derives the new window from the *particle* population (the
    "Unified Active-Range Union Focusing" pass), not from raw observations, so
    the particles here stand in for the posterior a real Bayesian update would
    have produced after converging on the dip.
    """
    ...
    # Posterior concentrated around the single dip (unit-space mean/std).
    center_unit = (dip_center - freq_lo) / (freq_hi - freq_lo)
    sigma_unit = dip_sigma / (freq_hi - freq_lo)
    smc._particles[:, 0] = np.clip(np.random.normal(loc=center_unit, scale=sigma_unit, size=1000), 0.0, 1.0)
    ...
    # Narrowing must produce a meaningfully smaller window. The exact width has
    # some run-to-run variance (resampling nudges/rejuvenation use an unseeded
    # internal RNG), so use a generous margin rather than a tight percentage.
    assert new_width < 0.85 * (freq_hi - freq_lo), "Window should have narrowed substantially"
...
def test_shoulder_based_narrowing_connected_dips():
    """Two adjacent dip posteriors are covered by one union window, not clipped to just one."""
    freq_lo, freq_hi = 2.7e9, 2.8e9
    smc = _make_smc(freq_lo, freq_hi)
    ...
    # Bimodal posterior: half the particles converged on each dip.
    ...
    particles = np.concatenate(
        [
            np.random.normal(loc=a_unit, scale=sigma_unit, size=half),
            np.random.normal(loc=b_unit, scale=sigma_unit, size=1000 - half),
        ]
    )
