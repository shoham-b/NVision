...
def test_focus_window_automatic_narrowing_during_resampling():
    ...
    # Narrowing is delayed until NVISION_MIN_STEPS_BEFORE_NARROWING (default 8) steps
    # have elapsed, so simulate having already explored for that long.
    smc._step_count = 8
...
def test_shoulder_based_narrowing_single_dip():
    """Window should shrink to span just the region the particle posterior has concentrated on.

    ``_resample()`` narrows based on the particle distribution (the "Unified Active-Range
    Union Focusing" percentile union over frequency/split/linewidth), not by walking
    observation shoulders directly, so we simulate a posterior that has already converged
    on the dip (as it would after several real Bayesian updates against dip observations)
    rather than feeding raw observations into a uniform particle set.
    """
    ...
    u_sigma = dip_sigma / (freq_hi - freq_lo)
    smc._particles[:, 0] = np.clip(np.random.normal(loc=u_center, scale=u_sigma, size=1000), 0.0, 1.0)
    # Keep the other (unconstrained) parameters tightly distributed too, so the joint
    # covariance used for resampling nudges stays well-conditioned.
    for j in (1, 2, 3):
        smc._particles[:, j] = np.clip(np.random.normal(loc=0.5, scale=0.01, size=1000), 0.0, 1.0)

    # Narrowing is delayed until NVISION_MIN_STEPS_BEFORE_NARROWING (default 8) steps have
    # elapsed, and particle rejuvenation injects a step-count-decaying fraction of fully
    # uniform particles on every resample. Use a step count high enough that this
    # rejuvenation fraction no longer swamps the narrow percentile tails being asserted on.
    smc._step_count = 60
    ...
    # Focusing on a converged single dip must produce a window well under 50% of the original span.
...
def test_shoulder_based_narrowing_connected_dips():
    """Two adjacent dips with no clear recovery between them are covered by one span.

    As above, we simulate a bimodal posterior (particles split between the two dip
    centres) rather than raw observations, since narrowing operates on the particle
    distribution.
    """
    freq_lo, freq_hi = 2.7e9, 2.8e9
    smc = _make_smc(freq_lo, freq_hi)
    ...
    smc._particles[:, 0] = np.clip(
        np.concatenate(
            [
                np.random.normal(loc=u_a, scale=u_sigma, size=half),
                np.random.normal(loc=u_b, scale=u_sigma, size=n - half),
            ]
        ),
        0.0,
        1.0,
    ...
    # See test_shoulder_based_narrowing_single_dip for why step_count must be this high.
    smc._step_count = 60
