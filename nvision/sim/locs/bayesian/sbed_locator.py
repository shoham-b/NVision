...
class SequentialBayesianExperimentDesignLocator(SequentialBayesianLocator):
    ...
    def _eig_acquire(self) -> float:
        ...
        # Cap to max_eig_candidates via uniform sub-sampling to bound EIG cost.
        if len(candidates) > self.max_eig_candidates:
            indices = np.round(np.linspace(0, len(candidates) - 1, self.max_eig_candidates)).astype(int)
