...
def nv_center_smc_belief(  # noqa: C901
    ...
    **_extra: object,
    ...
    ``frequency`` (the zero-field center) defaults to fixed -- a known, calibrated
    instrument constant rather than an inferred particle dimension, matching every
    ``NVCenterXModel``'s own default. Set ``with_fixed_frequency=False`` to make it a
    free/inferred particle dimension instead (e.g. for tests exercising the SMC scan-axis
    narrowing/expansion mechanics directly).
