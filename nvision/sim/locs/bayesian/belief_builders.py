...
def nv_center_smc_belief(  # noqa: C901
    ...
    **_extra: object,
    ...
    ``with_fixed_frequency=True`` (default) fixes ``frequency`` to the known
    ``NV_ZERO_FIELD_SPLITTING_HZ`` instrument constant instead of treating it as a
    free particle dimension -- set ``False`` to infer/localize it via SMC as before.
