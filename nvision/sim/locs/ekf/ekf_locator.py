...
class EKFLocator(SequentialBayesianLocator):
    ...
    def __init__(
        ...
        Args:
            ...
            initial_sweep_steps: Optional initial sweep steps.
    ...
    def create(
        ...
        **grid_config: object,
        ...
        bounds_phys = (
            dict(parameter_bounds) if parameter_bounds else nv_center_lorentzian_bounds_for_domain(2.6e9, 3.1e9)
        )
    ...
    def _on_sweep_complete(self) -> None:
        """Overridden to re-initialize EKF state and covariance from narrowed bounds."""
        super()._on_sweep_complete()

        # If the sweep was completely bypassed, preserve the Delta Method priors!
        if self.initial_sweep_steps == 0:
            return

        bounds = self.belief.physical_param_bounds
        priors = bounds.get("_priors") if isinstance(bounds, dict) else None

        freq_bounds = bounds.get("frequency", (self._acquisition_lo, self._acquisition_hi))
        ...
        # 3. Linewidth
        if priors and "linewidth" in priors:
            lw_val, lw_std = priors["linewidth"]
            Omega = lw_val / 1e6  # noqa: N806
            P_linewidth = (lw_std / 1e6) ** 2  # noqa: N806
        else:
            Omega = (linewidth_bounds[0] + linewidth_bounds[1]) / 2.0 / 1e6  # noqa: N806
            P_linewidth = ((linewidth_bounds[1] - linewidth_bounds[0]) / 1e6) ** 2 / 12.0  # noqa: N806

        # 4. Contrast k_NP and dip_depth a
        if priors and "k_np" in priors:
            knp_val, knp_std = priors["k_np"]
            k_NP = 1.0 / knp_val  # noqa: N806
            P_k_NP = (knp_std / (knp_val**2)) ** 2  # noqa: N806
        else:
            k_np_init = (k_np_bounds[0] + k_np_bounds[1]) / 2.0
            k_NP = 1.0 / k_np_init  # noqa: N806
            k_NP_hi = 1.0 / k_np_bounds[0]  # noqa: N806
        ...
        self.belief.theta_hat = np.array([f_B, delta_f_HF, a, Omega, k_NP])

        # Re-initialize covariance P
        self.belief.P = np.zeros((5, 5))
        self.belief.P[0, 0] = max(1e-6, P_freq)
        self.belief.P[1, 1] = max(1e-6, P_split)
        self.belief.P[2, 2] = max(1e-6, P_a)
        self.belief.P[3, 3] = max(1e-6, P_linewidth)
        self.belief.P[4, 4] = max(1e-6, P_k_NP)
    ...
    def observe(self, obs: Observation) -> None:
        ...
        if not self._staged_sobol.done():
            self._sweep_buffer.append(obs_physical)
            self._staged_sobol.observe(obs)  # Keep normalized for sobol
        else:
...
class EKFParticleFrequencyLocator(EKFLocator):
    ...
    def __init__(
        ...
        # Modify belief to use particles for frequency
        if hasattr(self.belief, "use_particles_for_frequency"):
            self.belief.use_particles_for_frequency()
        else:
            raise AttributeError("`use_particles_for_frequency` is not defined in EKFBelief.")

    @classmethod
    def create(cls, **config: object) -> EKFParticleFrequencyLocator:
        """Create EKFParticleFrequencyLocator enforcing particle-based frequency."""
        locator = super().create(**config)
        if hasattr(locator.belief, "use_particles_for_frequency"):
            locator.belief.use_particles_for_frequency()
        else:
            raise AttributeError("`use_particles_for_frequency` is not defined in EKFBelief.")
