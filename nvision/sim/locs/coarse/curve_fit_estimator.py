"""Shared least-squares curve-fit estimator for locators that collect (x, y) points.

Extracted from ``GenericSweepLocator`` (which pioneered this fit and is the
best-calibrated locator using it) so that other point-collecting locators --
``StagedSobolSweepLocator`` and ``SimpleSobolBayesianLocator`` -- can report the
same actual least-squares fit of the physical model to their own collected
observations as their final parameter estimate, instead of trusting a
batch-updated/resampled SMC belief marginal that can collapse onto a
plausible-but-wrong mode with a tiny reported uncertainty. Each locator keeps
its own acquisition strategy (uniform sweep, staged Sobol search, van der
Corput SMC) -- only the "turn collected points into a final estimate" step is
shared.
"""
...
# Parameters MAP-regularized in fit(), via _resolve_physical_priors below.
...
def _expected_frequency_crlb(
    ...
    Lorentzian; duplicated here because the fit works on raw physical points with
    no belief to query.  Returns ``inf`` when inputs are degenerate.
    """
...
class CurveFitResult:
    """Outcome of :meth:`CurveFitEstimator.fit`."""

    freq_phys: float
    uncert_phys: float
    fit_params_phys: dict[str, float] = field(default_factory=dict)


class CurveFitEstimator:
    """Fits a physical signal model to a set of ``(x, y)`` points via least squares.

    Stateless across calls except for ``fit_params_phys`` (the last fit's full
    parameter vector, kept for callers that want the whole fitted curve, not
    just the scan parameter). Any locator that has collected physical-unit
    observations can use this instead of trusting its own SMC belief's
    (possibly collapsed) posterior mode as its final estimate.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.fit_params_phys: dict[str, float] | None = None
    ...
    def fit(self, xs_norm: np.ndarray, ys: np.ndarray) -> CurveFitResult:
        """Fit the physical model to collected data via least squares.

        Returns ``(freq_phys, uncert_phys)`` wrapped in a :class:`CurveFitResult`,
        and stores the full fitted parameter vector on ``self.fit_params_phys``.
        The fit uses all points jointly, which gives sub-step accuracy even when
        dips are partially merged — much better than centroid heuristics. Raises
        if the model can't be fit; this estimator is only useful *because* of
        this fit, so a failure here is a real problem to surface, not a case to
        silently paper over with a cruder estimate.
        """
        ...
        # Store the full fitted parameter vector so callers can use the actual fit
        # instead of a (possibly collapsed) SMC belief marginal mode. scan_param
        # isn't part of best_popt when fixed, so add it back explicitly.
        fit_params_phys = {n: float(v) for n, v in zip(param_names, best_popt)}
        if scan_idx is None:
            fit_params_phys[scan_param] = freq_phys
        self.fit_params_phys = fit_params_phys
        ...
        return CurveFitResult(freq_phys=freq_phys, uncert_phys=uncert_phys, fit_params_phys=fit_params_phys)
    ...
    def _resolve_physical_bounds(self, param_names: list[str]) -> dict[str, tuple[float, float]]:
        ...
        Prefers ``signal_model.param_bounds_phys``, then the caller-provided
        ``parameter_bounds``, then per-parameter belief bounds. Raises if none are
        ...
        if hasattr(unit_model, "param_bounds_phys"):
            ...
            raise RuntimeError(
                ...
                "signal_model.param_bounds_phys, caller-provided parameter_bounds, "
    ...
    def _resolve_physical_priors(
        ...
        frequency from data is the entire point of the fit, and its prior is a coarse
        ...
        for name in param_names:
            if name == scan_param or name not in belief_priors or name not in _MAP_REGULARIZED_PARAMS:
                continue
    ...
    def _smooth_for_peak_detection(self, ys: np.ndarray) -> tuple[np.ndarray, int]:
        ...
        minimum feature span — a *physical* scale.  An uncapped window grows
        with sample count regardless of physics: dense data over a wide domain
        would produce a window many times wider than the dips themselves,
        smearing a whole hyperfine triplet into one blob so peak detection sees
        a single dip and the fit is seeded systematically wrong.
        ...
        if min_span is not None and min_span > 0 and domain_width > 0:
            ...
            # is finer than the data's own per-sample resolution, samples_per_min_span
            # collapses to ~0 and the cap floors the window to 3 regardless of this
            # draw's *actual* dip width -- badly under-smoothing a dip that's much
            # wider than the theoretical minimum, which corrupts hwhm_est and feeds
            # find_peaks spurious noise-scale features instead of the real dip edges.
            # A cap that can't resolve anything at this sampling density protects
            # against nothing, so skip it.
    ...
    def _seed_frequency_and_splits(
        ...
        *single* dip, but `frequency` is the *center* of the whole pattern —
        ...
        n_expected_dips = self._inner_model().expected_dip_count()
    ...
    def _run_curve_fit_candidates(
        ...
        *shape* parameters are still far from their optimum — frequency comes
        ...
        std — see ``fit``) with ``absolute_sigma=True`` so the two
    ...
    def _report_fit_uncertainty(
        ...
        contrast_fit = next(
            (self.fit_params_phys[n] for n in ("c_total", "c_max") if n in self.fit_params_phys),
            dip_depth,
        )
        crlb_at_fit = _expected_frequency_crlb(
            self.fit_params_phys.get("linewidth", lw_guess),
            contrast_fit,
