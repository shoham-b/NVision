    ...
    for repeat_idx, (repeat_entries, main_row) in enumerate(meta):
        ...
        for entry in repeat_entries:
            ...
            for field in _BULK_STRIP_FIELDS:
                slim.pop(field, None)
            if not entry.get("plot_skipped") and not entry.get("plots_pending"):
                # Entries whose figure was skipped by design (NVISION_PLOT_SWEEP_STRATEGIES) or is
                # still queued for a graph worker (plots_pending) keep their metadata/series but
                # have no graph to serve yet, so get no path.
