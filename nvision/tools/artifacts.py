...
def merge_locator_results_with_existing(
    ...
    try:
        ...
        # Make sure types are aligned for the join-key columns (used below in `key`
        # and the anti-join). Non-key numeric columns (e.g. "measurements") are left
        # alone here so the generic dtype-alignment pass below can correctly promote
        # Int64/Float64 mismatches instead of silently truncating to Int64.
        ...
        merged = pl.concat([old_df, df_loc], how="diagonal_relaxed").unique(
            subset=list(key),
            keep="last",
            maintain_order=True,
        )
