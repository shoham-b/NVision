"""Student's T Mixture belief with normalized unit-cube parameters and physical-scale public summaries."""
...
@dataclass
class UnitCubeStudentsTMixtureMarginalDistribution(StudentsTMixtureMarginalDistribution):
    """Like :class:`StudentsTMixtureMarginalDistribution` but internal state is on ``[0, 1]``.

    The ``model`` must be a :class:`UnitCubeSignalModel` mapping unit coordinates to
    the inner physical model. Internal means and uncertainties are in normalized
    ...
    def update(self, obs_norm: Observation) -> None:
        ...
        # In UnitCube mode, the model itself handles x mapping inside compute/gradient.
        # Since our 'model' IS a UnitCubeSignalModel, it expects normalized x.
