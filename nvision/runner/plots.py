...
def plots_wanted(strat_name: str, attempt_idx_in_combo: int) -> bool:
    """Whether figures should be built for this repeat.

    False for the sweep strategies unless ``NVISION_PLOT_SWEEP_STRATEGIES`` is set, and for
    repeats at or beyond ``NVISION_GRAPH_REPEATS`` (when that limit is positive).
    """
    if strat_name in _SWEEP_STRATEGY_NAMES and not NVISION_PLOT_SWEEP_STRATEGIES:
        return False
    return not (NVISION_GRAPH_REPEATS > 0 and attempt_idx_in_combo >= NVISION_GRAPH_REPEATS)
...
def generate_attempt_plots(
    ...
    With ``defer=True`` no figures are built: only the slim scan entry (metrics, per-step
    series, true params) is returned, marked ``plots_pending``, so the caller can save the
    repeat immediately and hand the plot inputs to a graph worker (see
    ``nvision/runner/graph_queue.py``), which calls this again with ``defer=False``.
    """
    ...
    if strat_name not in ("SimpleSobol", "SimpleSweep") and wants_baselines:
        try:
    ...
    plots_skipped = not plots_wanted(strat_name, attempt_idx_in_combo)
    plots_deferred = defer and not plots_skipped
    no_figures = plots_skipped or plots_deferred
    if no_figures:
        # No figure: drop the path so nothing tries to embed or serve a plot that was never
        # built, and flag the entry so the UI/server can tell "skipped by design" (plot_skipped)
        # or "a graph worker will build it" (plots_pending) from "missing".
        scan_entry.pop("path", None)
        scan_entry["plot_skipped" if plots_skipped else "plots_pending"] = True
    else:
    ...
    if run_result is not None and _is_bayesian_run(strat_name, strat_obj) and not no_figures:
        try:
            entries.extend(
