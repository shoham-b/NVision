...
function main() {
    ...
    // Short label used both in the per-axis dropdown and the family heading.
    const GRID_AXIS_INFO = {
        width: { label: 'Width (MHz)', short: 'width', fromMetrics: (m) => (m.grid_linewidth != null ? m.grid_linewidth / 1e6 : null) },
        contrast: { label: 'Contrast', short: 'contrast', fromMetrics: (m) => m.grid_c_total },
        saturation: { label: 'Saturation', short: 'saturation', fromMetrics: (m) => m.grid_saturation },
        sigma_inhom: { label: 'sigma_inhom (MHz)', short: 'sigma_inhom', fromMetrics: (m) => (m.grid_sigma_inhom != null ? m.grid_sigma_inhom / 1e6 : null) },
    };
    // Order controls both the axis dropdown order and the family label order.
    const GRID_AXIS_ORDER = ['width', 'contrast', 'saturation', 'sigma_inhom'];

    // Maps generator name -> parsed facets, built once from scanPlots' structured
    // metrics.grid_* fields (see nvision/runner/metrics.py) rather than by parsing
    // the generator name string. Every scan entry for a given generator carries the
    // same grid_* values (they're fixed per-generator, not per-repeat), so any one
    // matching entry is enough to populate the whole generator's facets.
    let generatorFacetsIndexCache = null;
    let generatorFacetsIndexSourceLen = -1;
    function getGeneratorFacetsIndex() {
        if (generatorFacetsIndexCache && generatorFacetsIndexSourceLen === scanPlots.length) {
            return generatorFacetsIndexCache;
        }
        const idx = new Map();
        for (const p of scanPlots) {
            if (idx.has(p.generator) || !p.metrics) continue;
            const m = p.metrics;
            const variant = m.grid_variant;
            if (!variant) continue;
            const axes = [];
            for (const key of GRID_AXIS_ORDER) {
                const value = GRID_AXIS_INFO[key].fromMetrics(m);
                if (value != null) axes.push({ key, label: GRID_AXIS_INFO[key].label, value });
            }
            if (!axes.length) continue;
            const family = `${variant}_${axes.map((a) => a.key).join('_')}`;
            const familyLabel = `${variant} (${axes.map((a) => GRID_AXIS_INFO[a.key].short).join(' × ')})`;
            idx.set(p.generator, { family, familyLabel, axes });
        }
        generatorFacetsIndexCache = idx;
        generatorFacetsIndexSourceLen = scanPlots.length;
        return idx;
    }

    // Fallback for manifests generated before metrics.py started recording
    // grid_variant/grid_* (older cached runs re-rendered without a fresh
    // simulation won't have it backfilled). Parses the same information back out
    // of the generator name string this legacy data was known to follow. Once a
    // manifest is regenerated by a fresh run, getGeneratorFacetsIndex() picks it
    // up structurally and this path is never reached for it.
    function parseGeneratorFacetsFromNameLegacy(name) {
        let m = /^NVCenter-saturation_voigt-c([\d.]+)-si([\d.]+)MHz$/.exec(name);
        if (m) {
            return {
                family: 'saturation_voigt_saturation_sigma_inhom',
                familyLabel: 'saturation_voigt (saturation × sigma_inhom)',
                axes: [
                    { key: 'saturation', label: 'Saturation', value: Number(m[1]) },
                    { key: 'sigma_inhom', label: 'sigma_inhom (MHz)', value: Number(m[2]) },
                ],
            };
        }
        m = /^NVCenter-([a-zA-Z_]+)-w([\d.]+)MHz-c([\d.]+)-si([\d.]+)MHz$/.exec(name);
        if (m) {
            const variant = m[1];
            return {
                family: `${variant}_width_contrast_sigma_inhom`,
                familyLabel: `${variant} (width × contrast × sigma_inhom)`,
                axes: [
                    { key: 'width', label: 'Width (MHz)', value: Number(m[2]) },
                    { key: 'contrast', label: 'Contrast', value: Number(m[3]) },
                    { key: 'sigma_inhom', label: 'sigma_inhom (MHz)', value: Number(m[4]) },
                ],
            };
        }
    ...
    // Look up the swept axes for a generator produced by a parameter-grid study, so
    // the UI can offer one dropdown per axis instead of a flat list of opaque
    // generator names. Returns null for non-grid generators (e.g. bare
    // "NVCenter-lorentzian", which has no grid_* values set). Prefers the
    // structured metrics.grid_* data; falls back to name parsing only for
    // manifests predating that field (see parseGeneratorFacetsFromNameLegacy).
