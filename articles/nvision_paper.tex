...
Like the Lorentzian family, this triplet is population-normalized, reusing the same $(p_L,p_0,p_R)$ weights
of Eq.~\eqref{eq:triplet-pop} (unified onto this convention 2026-07-15 — an earlier per-dip \code{dip\_depth}
convention, not population-normalized, is retained only for the single, non-degenerate zero-field peak,
\code{NVCenterOnePeakLorentzianSpectrum}):
\begin{widetext}
\begin{equation}
  S(x) = 1 - \Big[p_L\, V(x-f_0+\Delta f) + p_0\, V(x-f_0) + p_R\, V(x-f_0-\Delta f)\Big],
\end{equation}
\end{widetext}
\noindent so total contrast $c$ stays fixed as $k_{\rm np}$ varies, exactly as in Eq.~\eqref{eq:triplet}. The
saturation-coupled model below reuses this same population kernel ($\Delta f_{\rm Zeeman}{=}0$ when Zeeman
splitting is off).
...
\colrule
Center frequency domain & $f_0$ & $2.72$--$3.02$~GHz \\
...
Zeeman half-separation & $\Delta f_{\rm Zeeman}$ & $0$--$60$~MHz ($\approx$ 0--2.1~mT) \\
...
4.5$\times$ the measurements. $f_\sigma=0.25$ was calibrated by replaying the rule inside full-budget runs,
firing on 100\% of both ordinary and degenerate configurations and saving $2.3\times$ and $3.6\times$
measurements respectively (median step 450$\to$197 ordinary, 450$\to$124 degenerate). A separate live A/B
(120 configs/arm) confirmed the rule end-to-end rather than by replay, giving a close but distinct
450$\to$220 ordinary (124 degenerate, unchanged); accuracy on most target parameters stayed within a few
percent of the full-budget run — well under the $\sqrt2$--$\sqrt{3.6}$ increase a naive $\sqrt n$ scaling
would predict. The one measured cost, from that same live A/B: on degenerate configurations the
catastrophic-error rate rises 7.1\%$\to$12.5\% (4$\to$7 of 56 repeats); a more conservative $f_\sigma=0.15$
keeps most of that margin at a $2.2\times$ rather than $3.6\times$ saving.
