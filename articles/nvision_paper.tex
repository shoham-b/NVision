...
($K_{\rm safety}=4.0$), stopping as infeasible if $n_{\rm req}>N_{\max}K_{\rm safety}$. In SBED, FIM-based
CRLBs are rescaled by the online noise estimate, $\mathrm{CRLB}_j^{\rm scaled}=\mathrm{CRLB}_j^{\rm FIM}
\hat\sigma_{\rm bg}/\sigma_{\rm nominal}$, and convergence requires $\sigma_j<K_{\rm safety}
\mathrm{CRLB}_j^{\rm scaled}$.
...
either prior attempt or \citet{dushenko2020sbed}, and (ii) using them as the actual stopping/feasibility
mechanism in place of fixed-iteration or fixed-tolerance rules — arguably the paper's most concrete
...
(Section~\ref{sec:sbed} count exceeded); \code{max\_steps} (budget exhausted otherwise).
